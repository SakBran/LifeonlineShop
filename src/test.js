...
MySvgFonts.prototype.hasFont = function(name) {
  if (
    typeof this.fonts[name] != "undefined" &&
...
var mySvgFont = {
  ...
  renderSvg: function(svg, fontName, text, size, color, background) {
    ...
    if (color != undefined) scaleG.setAttribute("fill", color);
    var width = 0;
    var lineCount = 1;
    var uText = "u";
    var gData;
    for (var i = 0; i < text.length; i++) {
      ...
      for (var j = i; j < text.length && j < i + mySvgFont.maxContext; j++) {
        ...
        try {
          ...
          if (mySvgFont.fontWarning)
            alert("Error parsing " + text + " at " + i);
          mySvgFont.fontWarning = false;
        }
      }
      if (lastMatchIndex > -1) {
